...
function MultiSelectPopover({ options, selected, onChange, placeholder }: { options: Array<{ value: string; label: string }>; selected: string[]; onChange: (values: string[]) => void; placeholder: string }) {
  ...
  return <div className="multi-select"><button type="button" className={`multi-select-trigger ${selected.length ? "has-value" : ""}`} onClick={() => { setDraft(selected); setOpen((current) => !current); }}>{selected.length ? `${selected.length} selecionada(s)` : placeholder}<span aria-hidden>▾</span></button>{open && <div className="multi-select-menu"><div className="multi-select-options">{options.map((option) => <label key={option.value}><input type="checkbox" checked={draft.includes(option.value)} onChange={() => toggle(option.value)} /><span>{option.label}</span></label>)}</div><div className="multi-select-actions"><button type="button" onClick={() => setDraft([])}>Limpar</button><button type="button" className="primary-button" onClick={() => { onChange(draft); setOpen(false); }}>Concluído</button></div></div>}</div>;
}
